<template>
  <view @touchmove.stop.prevent>
    <view class="tui-reply-tabbar" :style="{ paddingBottom: `${keyboardHeight}px` }">
      <view class="tui-chat-tabbar">
        ...
        <view class="tui-send-box">
          ...
          <view class="tui-btn-send" v-if="content" :style="{ color: color }" hover-class="tui-opcity" :hover-stay-time="150" @tap="hideKeyboard">{{ sendText }}</view>
        </view>
      </view>
    ...
    <view class="tui-reply-mask" v-if="mask" :class="{ 'tui-box-show': showIndex, 'tui-mask-show': showIndex && !maskOpacity }" @touchmove.stop.prevent @tap="hideKeyboard"></view>
  </view>
</template>
