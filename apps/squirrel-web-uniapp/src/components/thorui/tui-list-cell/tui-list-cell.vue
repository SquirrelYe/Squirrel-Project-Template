<template>
  <view
    ...
    :class="[radius && radius != '0' ? 'tui-radius' : '', hover ? 'tui-cell-hover' : '']"
    :style="{ backgroundColor: backgroundColor, fontSize: getSize + 'rpx', color: getColor, padding: getPadding, borderRadius: radius + 'rpx' }"
    @tap="handleClick"
  >
    <slot></slot>
    <view class="tui-cell__line" :style="{ borderBottomColor: getLineColor, left: getLineLeft + 'rpx', right: lineRight + 'rpx' }" v-if="!unlined"> </view>
