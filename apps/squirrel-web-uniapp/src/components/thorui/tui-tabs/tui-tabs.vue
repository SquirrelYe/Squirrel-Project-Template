<template>
  <view
    ...
    v-if="tabsWidth > 0"
  >
    <view v-for="(item, index) in tabs" :key="index" class="tui-tabs-item" :style="{ width: getItemWidth, height: height + 'rpx' }" @tap.stop="swichTabs(index)">
      <view
      ...
      >
        {{ item.name }}
        <view :class="[item.isDot ? 'tui-badge__dot' : 'tui-tabs__badge']" :style="{ color: badgeColor, backgroundColor: getBadgeBgColor }" v-if="item.num || item.isDot">
          {{ item.isDot ? '' : item.num }}
        </view>
      </view>
    </view>
