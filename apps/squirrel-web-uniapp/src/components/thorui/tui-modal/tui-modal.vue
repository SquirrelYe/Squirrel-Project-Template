      ...
      <view v-if="!custom">
        ...
        <view class="tui-modalBtn-box" :class="[button.length != 2 ? 'tui-flex-column' : '']">
          <view
            ...
            :key="index"
            class="tui-modal-btn"
            :class="[button.length != 2 ? 'tui-btn-width' : '', button.length > 2 ? 'tui-mbtm' : '', shape == 'circle' ? 'tui-circle-btn' : '']"
            :style="{ background: getColor(item.type, true, item.plain), color: getTextColor(item.type, item.plain) }"
            @tap="handleClick(index)"
          >
            {{ item.text || '确定' }}
            <view class="tui-modal__border" :class="[shape == 'circle' ? 'tui-circle-border' : '']" :style="{ borderColor: getColor(item.type) }" v-if="item.plain"></view>
          </view>
        </view>
      </view>
      <view v-else>
