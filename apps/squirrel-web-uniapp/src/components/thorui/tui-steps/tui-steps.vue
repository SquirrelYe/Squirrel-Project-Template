    ...
    >
      <view class="tui-step-item-ico" :class="[direction === 'column' ? 'tui-step-column_ico' : 'tui-step-row_ico']" :style="{ width: direction === 'column' ? '36rpx' : '100%' }">
        ...
        <view class="tui-step-custom" :style="{ backgroundColor: backgroundColor }" v-if="item.name || item.icon">
          <tui-icon :name="item.name" :size="item.size || 20" :color="index <= activeSteps ? getActiveColor : deactiveColor" v-if="item.name"></tui-icon>
      ...
      <view class="tui-step-item-main" :class="['tui-step-' + direction + '_item_main']">
        ...
        <view class="tui-step-item-content" :style="{ color: index <= activeSteps ? getActiveColor : deactiveColor, fontSize: descSize + 'rpx' }"> {{ item.desc }}</view>
      </view>
    </view>
  </view>
</template>
